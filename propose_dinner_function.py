...
#propeses a dinner from all the dinner which are stored in the databank. Based just on a random number.
...
def import_recipe():
    cursor.execute('SELECT dinner FROM calorie_data WHERE rowid=1')
    x = cursor.fetchone()
    einzuhaltende_kalorien = x[0]       #gets the allowed calories for dinner

    cursor.execute("SELECT name FROM dinner_lunch_recipes")     #gets a dinner
    ...
    ingredients = "SELECT ingredients FROM dinner_lunch_recipes WHERE rowid=?"  #selects the ingredients from the chosen dinner
    # and calculates the amount of ingredients to reach your allowed calories
