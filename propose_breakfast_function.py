...
#propeses a breakfast from all the breakfast which are stored in the databank. Based just on a random number.
...
def import_recipe():
    cursor.execute('SELECT breakfast FROM calorie_data WHERE rowid=1')
    x = cursor.fetchone()
    einzuhaltende_kalorien = x[0]           #gets the allowed calories for breakfast


    cursor.execute("SELECT name FROM breakfast_recipes")        #gets a breakfast
    zufall_zahl = random.randint(1,int(len(cursor.fetchall())))
    name = "SELECT name FROM breakfast_recipes WHERE rowid=?"
    cursor.execute(name,[zufall_zahl])
    zufall_name = cursor.fetchone()[0]

    ingredients = "SELECT ingredients FROM breakfast_recipes WHERE rowid=?"     #calculates the amount of ingredients to reach your allowed calories
    ...
    kalories = "SELECT kcal FROM breakfast_recipes WHERE rowid=?"
    cursor.execute(kalories,[zufall_zahl])
    kalorien = cursor.fetchone()[0]
    multiplikator_kalorien = einzuhaltende_kalorien/kalorien    #berechnet inwieweit die Portion vergrößert werden muss
