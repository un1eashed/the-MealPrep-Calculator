...
def import_recipe():
    ...
    snack_anzahl = 0    #now the amount of snacks is calculated for one day based on what was calculated in the calculation window ("module: GUI_calculate_calories.py")
    if einzuhaltende_kalorien == 0:
        ...
        snack_anzahl = 2
        einzuhaltende_kalorien = einzuhaltende_kalorien/snack_anzahl
    ...
    cursor.execute("SELECT name FROM snacks_recipes")
    zufall_zahl = random.randint(1,int(len(cursor.fetchall())))
    name = "SELECT name FROM snacks_recipes WHERE rowid=?"
    cursor.execute(name,[zufall_zahl])
    zufall_name = cursor.fetchone()[0]

    ingredients = "SELECT ingredients FROM snacks_recipes WHERE rowid=?"
    ...
    kalories = "SELECT kcal FROM snacks_recipes WHERE rowid=?"
    ...
    for i in zutatenliste: #nimmt die einzelnen Zutaten, extrahiert die Zahl, multipliziert diese mit dem Faktor aus dem Kalorienverbrauch
        ...
        try:
            ...
            angepasster_wert = anzupassender_wert * multiplikator_kalorien #divided by the amount of snacks you want to eat a day
